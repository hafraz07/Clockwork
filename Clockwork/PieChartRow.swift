...
public struct PieChartRow : View {
    ...
    var slices: [PieSlice] {
        ...
        for i in 0...pieData.data.count - 1 {
            let normalized:Double = Double(pieData.data[i])/Double(maxValue)
            let startDeg = lastEndDeg
            let endDeg = lastEndDeg + (normalized * 360)
            lastEndDeg = endDeg
            tempSlices.append(PieSlice(startDeg: startDeg, endDeg: endDeg, value: pieData.data[i], normalizedValue: normalized, slideColor: pieData.activities[i].slideColor))
        }
        return tempSlices
    }
        ...
        GeometryReader { geometry in
            ZStack{
                ForEach(0..<self.slices.count){ i in
                    PieChartCell(rect: geometry.frame(in: .local), startDeg: self.slices[i].startDeg, endDeg: self.slices[i].endDeg, index: i, backgroundColor: self.backgroundColor, accentColor: self.accentColor, foregroundColor: self.slices[i].slideColor)
